...
export default function PageVine() {
  ...
  return (
    <div
      ref={containerRef}
      aria-hidden
      className="pointer-events-none absolute left-0 top-0 bottom-0 hidden w-3 lg:block"
    >
      <svg
        viewBox="0 0 8 100"
        preserveAspectRatio="none"
        className="h-full w-full overflow-visible text-[color:var(--rule)]"
        fill="none"
        stroke="currentColor"
        strokeWidth="1"
        strokeLinecap="round"
      ...
      {leafOffsets.map((offset, i) => (
        <span
          key={offset}
          className={`absolute -left-1 text-[color:var(--palm-leaf)] transition-opacity duration-500 ${
